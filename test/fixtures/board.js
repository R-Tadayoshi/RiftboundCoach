...
function build(overrides) {
  ...
  const sideHtml = (side) => `
    <section data-zone-owner="${side}">
      <div data-drop-zone="legend" data-card-id="legend-${side}">
        <button data-drop-zone="legend" data-card-id="legend-${side}" data-board-card-visual="true">
          <img alt="${side === "self" ? o.selfLegend : o.opponentLegend}" src="${ART("OGN-001")}">
        </button>
      </div>
      <section data-drop-zone-root="champion" data-visual-owner="${side}">
